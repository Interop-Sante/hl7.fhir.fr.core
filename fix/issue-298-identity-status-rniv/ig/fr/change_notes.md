...
| `RECUP`**(nouveau)** | Identité récupérée | + | − |
| `VALI` | Identité validée | − | + |
| `QUAL`**(nouveau)** | Identité qualifiée | + | + |

* `fr-core-cs-identity-status-comment` (`FRCoreCodeSystemIdentityStatusComment`) **(nouveau)** : les 19 autres codes de l’ancien `fr-core-cs-v2-0445` (attributs RNIV — homonyme, fictif, douteux — et codes de gestion — doublon, désactivé, collision…), destinés à la sous-extension `comment`.
