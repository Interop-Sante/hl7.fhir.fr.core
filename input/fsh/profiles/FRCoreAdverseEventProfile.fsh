...
// ─────────────────────────────────────────────────────────────────────────────
// Extensions locales (backport R5→R4) : AdverseEvent.code et AdverseEvent.outcome
// n'ont pas d'extension cross-version officielle dans l'IG xver R5→R4
// (https://hl7.org/fhir/uv/xver-r5.r4/0.1.0/artifacts.html ne couvre pas
// AdverseEvent.code ni AdverseEvent.outcome), contrairement au pattern habituel
// $xxx-r5 (alias vers http://hl7.org/fhir/5.0/StructureDefinition/extension-...).
// Elles sont donc définies ici et leur ^url fixé au format officiel des
// backports R5→R4 (http://hl7.org/fhir/5.0/StructureDefinition/extension-
// <Resource>.<Element>, ex. extension-AdverseEvent.contributingFactor), même si
// HL7 ne publie pas lui-même de définition à cette adresse pour code/outcome
// (vérifié absente de hl7.fhir.uv.xver-r5.r4, hl7.fhir.uv.extensions et 404 sur
// hl7.org).
// ─────────────────────────────────────────────────────────────────────────────

// Mime AdverseEvent.code (R5) — renommage de AdverseEvent.event (R4, binding
// Example SNOMED CT) sans équivalent bindable en R4 pour le JDV français.
...
// Mime AdverseEvent.outcome (R5, binding Example) — en R4 le binding est
// Required vers un valueset HL7 figé, donc impossible à re-binder sur
// jdv-evolution-cisis. Non couvert par l'IG cross-version xver-r5.r4.
...
// Fixé à medication-mishap : ce code vient de la révision R5 du CodeSystem
// terminology.hl7.org/CodeSystem/adverse-event-category (partagé R4/R5, binding
// extensible dès R4) — FRCore/Doc Core AdverseEvent ne couvre que les effets
// indésirables médicamenteux (cf. https://www.hl7.org/fhir/adverseevent.html).
// Le binding JDV d'origine (jdv-origine-effet-indesirable-cisis) est déplacé
// vers l'extension "code" (backport R5→R4, voir en tête de fichier).
...
// Extension backport mimant AdverseEvent.code (R5) — voir définition en tête
// de fichier. Porte le JDV d'origine, précédemment (à tort) sur category.
...
// event (R4 natif, binding Example SNOMED CT) neutralisé : remplacé par
// l'extension "code" qui reprend le rôle de AdverseEvent.code (R5, renommage
// de event) pour porter le JDV d'origine.
* event 0..0
...
// outcome (R4 natif) neutralisé : binding Required vers un valueset HL7 figé,
// incompatible avec jdv-evolution-cisis. Remplacé par l'extension "outcome"
// (backport R5→R4, voir définition en tête de fichier) qui mime le binding
// Example de R5, librement re-bindable.
* outcome 0..0
